<x-app-layout>
    <x-slot name="header">
        {{-- Usamos exactamente las mismas clases que en el Panel de Usuarios --}}
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Card: Cuentas por Aprobar --}}
                    <div class="bg-white dark:bg-[#212121]/80 backdrop-blur-md border border-gray-100 dark:border-white/10 rounded-3xl shadow-xl p-6 flex flex-col justify-between hover:bg-white/50 dark:hover:bg-[#212121] transition-all duration-300 group">
                        <div class="flex items-start gap-4">
                            <div class="p-4 bg-smart-warning/10 dark:bg-smart-warning/20 rounded-2xl text-smart-warning shadow-sm border border-smart-warning/20">
                                <span class="material-symbols-outlined text-4xl group-hover:rotate-12 transition-transform duration-300">how_to_reg</span>
                            </div>
                            <div>
                                <h4 class="text-smart-text dark:text-white font-black text-xl mb-1 transition-colors">Cuentas por Aprobar</h4>
                                <p class="text-smart-text/70 dark:text-smart-niebla/70 text-sm font-medium leading-relaxed transition-colors">
                                    Tienes <span class="px-2 py-0.5 bg-smart-warning/10 dark:bg-smart-warning/20 text-smart-warning rounded-lg font-black">{{ $pendientesAprobar }}</span> ciudadanos esperando validación.
                                </p>
                            </div>
                        </div>
                        <div class="mt-8">
                            <a href="{{ route('admin.usuarios') }}" class="flex items-center justify-center gap-2 w-full py-3 bg-smart-action text-white text-xs font-black uppercase rounded-xl transition-all shadow-lg hover:shadow-smart-action/40 hover:-translate-y-1 active:scale-95">
                                Revisar Solicitudes <span class="material-symbols-outlined text-[18px]">arrow_forward</span>
                            </a>
                        </div>
                    </div>

                    {{-- Card: Nuevas Incidencias --}}
                    <div class="bg-white dark:bg-[#212121]/80 backdrop-blur-md border border-gray-100 dark:border-white/10 rounded-3xl shadow-xl p-6 flex flex-col justify-between hover:bg-white/50 dark:hover:bg-[#212121] transition-all duration-300 group">
                        <div class="flex items-start gap-4">
                            <div class="p-4 bg-smart-action/10 dark:bg-smart-action/20 rounded-2xl text-smart-action shadow-sm border border-smart-action/20">
                                <span class="material-symbols-outlined text-4xl group-hover:scale-110 transition-transform duration-300">campaign</span>
                            </div>
                            <div>
                                <h4 class="text-smart-text dark:text-white font-black text-xl mb-1 transition-colors">Nuevas Incidencias</h4>
                                <p class="text-smart-text/70 dark:text-smart-niebla/70 text-sm font-medium leading-relaxed transition-colors">
                                    Hay <span class="px-2 py-0.5 bg-smart-action/10 dark:bg-smart-action/20 text-smart-action rounded-lg font-black">{{ $reportesNuevos }}</span> reportes recientes sin atender.
                                </p>
                            </div>
                        </div>
                        <div class="mt-8">

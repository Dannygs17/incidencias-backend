<x-app-layout>
    ...
    <div class="py-12" x-data="{ openModal: false, openMap: false, reporteSeleccionado: {}, previewUrl: null }">
        ...
        <div x-show="openMap" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm" style="margin: 0;" x-cloak>
            <div class="bg-gray-900 border border-gray-800 w-full max-w-3xl shadow-2xl rounded-xl overflow-hidden" @click.away="openMap = false">
                ...
                <div class="p-4 bg-gray-900 border-t border-gray-800 text-center">
                    <button @click="openMap = false" class="bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-6 rounded-lg transition-all text-sm uppercase">Cerrar Mapa</button>
                </div>
            </div>
        </div>
        ...
        <div x-show="openModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm" style="margin: 0;" x-cloak>
            <div class="bg-gray-900 border border-gray-800 w-full max-w-2xl shadow-2xl rounded-xl overflow-hidden max-h-[90vh] flex flex-col" @click.away="openModal = false">
                
                <div class="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-900 shrink-0">
                    <h3 class="text-xl text-white font-black uppercase tracking-tight">Gestión del Reporte</h3>
                    <button @click="openModal = false" class="text-gray-500 hover:text-red-500 transition font-bold text-2xl px-2">×</button>
                </div>
                
                <div class="p-6 overflow-y-auto">
                    
                    <h4 class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Evidencia Ciudadana</h4>
                    ...
                    <div class="space-y-3 mb-6">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-widest">Descripción</p>
                    ...
                    <template x-if="reporteSeleccionado.estado === 'resuelto'">
                        <div class="mt-6 border-t border-gray-800 pt-6">
                            <h4 class="text-sm font-bold text-green-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                <span class="material-symbols-outlined">verified</span> Respuesta Oficial
                            </h4>
                            
                            <template x-if="reporteSeleccionado.evidencia_path">
                                <div class="w-full h-48 bg-gray-800 rounded-lg mb-4 flex items-center justify-center border border-green-500/30 shadow-inner overflow-hidden">
                                    <img :src="'/storage/' + reporteSeleccionado.evidencia_path" class="w-full h-full object-cover">
                                </div>
                            </template>
                            
                            <template x-if="reporteSeleccionado.comentario_admin">
                                <p class="text-gray-300 leading-relaxed border-l-4 border-green-500 pl-4 bg-green-900/20 py-3 rounded-r" x-text="reporteSeleccionado.comentario_admin"></p>
                            </template>
                            
                            <template x-if="!reporteSeleccionado.evidencia_path && !reporteSeleccionado.comentario_admin">
                                <p class="text-gray-500 italic text-sm">Resuelto sin comentarios ni evidencia fotográfica adicional.</p>
                            </template>
                        </div>
                    </template>
                ...
                <div class="p-4 bg-gray-900 border-t border-gray-800 shrink-0">
                    ...
                    <template x-if="reporteSeleccionado.estado === 'pendiente'">
                        <form :action="'/admin/incidencia/' + reporteSeleccionado.id + '/estado'" method="POST" class="w-full">
                            ...
                            <button type="submit" class="w-full bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 font-black py-3 rounded-lg transition-all uppercase text-xs shadow-md">
                                Asignar a Cuadrilla (En Proceso)
                            </button>
                        </form>
                    </template>

                    <template x-if="reporteSeleccionado.estado === 'en proceso'">
                        <form :action="'/admin/incidencia/' + reporteSeleccionado.id + '/estado'" method="POST" enctype="multipart/form-data" class="w-full space-y-4">
                            @csrf
                            <input type="hidden" name="nuevo_estado" value="resuelto">
                            
                            <div class="bg-gray-800 p-4 rounded-lg border border-gray-700">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Comentario Oficial (Opcional)</label>
                                <textarea name="comentario_admin" rows="2" class="w-full bg-gray-900 border border-gray-700 text-gray-200 rounded-md focus:ring-green-500 focus:border-green-500 text-sm placeholder-gray-600" placeholder="Ej: Fuga reparada por la cuadrilla #4..."></textarea>
                                
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 mt-4">Foto de Evidencia (Opcional)</label>
                                <input type="file" name="evidencia" accept="image/*" class="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-gray-700 file:text-gray-200 hover:file:bg-gray-600" 
                                       @change="const file = $event.target.files[0]; if(file) previewUrl = URL.createObjectURL(file);">
                                
                                <template x-if="previewUrl">
                                    <div class="mt-3 w-32 h-32 rounded-lg border border-green-500 overflow-hidden shadow-lg">
                                        <img :src="previewUrl" class="w-full h-full object-cover">
                                    </div>
                                </template>
                            </div>

                            <button type="submit" class="w-full bg-green-600 hover:bg-green-500 text-white font-black py-3 rounded-lg transition-all uppercase text-xs shadow-md flex justify-center items-center gap-2">
                                <span class="material-symbols-outlined text-sm">check_circle</span> Marcar como Resuelto y Notificar
                            </button>
                        </form>
                    </template>
                    ...
                    <template x-if="reporteSeleccionado.estado === 'resuelto'">
                         <div class="w-full text-center py-2 text-green-500 font-bold text-sm uppercase flex justify-center items-center gap-2">
                             <span class="material-symbols-outlined">verified</span> Caso Cerrado Exitosamente
                         </div>
                    </template>
                </div>
            </div>

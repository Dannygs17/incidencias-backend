<x-app-layout>
    ...
    <div class="py-12" x-data="{ openModal: false, openMap: false, reporteSeleccionado: {}, previewUrl: null }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div class="mb-6 p-4 bg-smart-success/10 border border-smart-success/20 text-smart-success rounded-xl shadow-sm font-bold flex items-center">
            ...
            <div class="bg-white dark:bg-[#212121] border border-gray-100 dark:border-white/5 rounded-2xl shadow-xl overflow-hidden">
                
                {{-- Navegación de Estados (Tabs) --}}
                <div class="flex border-b border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-black/20 overflow-x-auto">
                    <a href="{{ route('admin.tabla_incidencias', ['categoria' => $categoria->id, 'estado' => 'pendiente']) }}" 
                       class="px-8 py-4 border-r border-gray-100 dark:border-white/5 min-w-max transition text-xs font-black uppercase tracking-tighter
                ...
                <div class="p-6">
                    <div class="border border-gray-100 dark:border-white/5 rounded-xl overflow-hidden shadow-inner bg-gray-50/50 dark:bg-black/10">
                        <table class="w-full text-left text-sm border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                                ...
                                <tr class="hover:bg-white dark:hover:bg-white/5 transition group">
                                    <td class="p-4 font-bold text-gray-700 dark:text-white">
                                    ...
                                    <td class="p-4 text-center text-gray-400 font-medium text-xs">
                                    ...
                                    <td class="p-4 text-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black border uppercase tracking-wider {{ $colores[$incidencia->estado] }}">
                                            {{ $incidencia->estado }}
                                        </span>
                                    </td>
                                    <td class="p-4 text-center">
        ...
        <div x-show="openMap" class="fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-4 backdrop-blur-md" x-cloak>
            <div class="bg-white dark:bg-[#212121] border dark:border-white/10 w-full max-w-4xl shadow-2xl rounded-[2rem] overflow-hidden" @click.away="openMap = false">
                <div class="p-6 border-b dark:border-white/5 flex justify-between items-center bg-gray-50 dark:bg-black/40">
                    <h3 class="text-xl text-smart-cobalto-dark dark:text-white font-black uppercase tracking-tighter flex items-center gap-2">
                        <span class="material-symbols-outlined text-smart-action">map</span> Ubicación del Reporte
                    </h3>
                    <button @click="openMap = false" class="text-gray-400 hover:text-smart-error transition text-3xl font-light">×</button>
                </div>
                <div class="p-4">
                    <div id="mapAdmin" class="w-full h-[65vh] rounded-2xl border dark:border-white/5 z-0 relative shadow-inner"></div>
                </div>
            </div>
        </div>
        ...
        <div x-show="openModal" class="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4 backdrop-blur-md" x-cloak>
            <div class="bg-white dark:bg-[#212121] border dark:border-white/10 w-full max-w-2xl shadow-2xl rounded-[2rem] overflow-hidden max-h-[90vh] flex flex-col" @click.away="openModal = false">
                ...
                <div class="p-6 border-b dark:border-white/5 flex justify-between items-center bg-gray-50 dark:bg-black/40 shrink-0">
                    <h3 class="text-xl text-smart-cobalto-dark dark:text-white font-black uppercase tracking-tighter">Detalles de Incidencia</h3>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <template x-if="reporteSeleccionado.imagen_path">
                                <div class="w-full h-56 bg-gray-100 dark:bg-black/40 rounded-2xl border dark:border-white/5 overflow-hidden shadow-inner group">
                            ...
                            <template x-if="!reporteSeleccionado.imagen_path">
                                <div class="w-full h-56 bg-gray-50 dark:bg-black/20 rounded-2xl flex flex-col items-center justify-center border-2 border-dashed dark:border-white/5 text-gray-400">
                                    <span class="material-symbols-outlined text-5xl mb-2 opacity-20">no_photography</span>
                                    <span class="text-[10px] font-bold uppercase tracking-widest">Sin evidencia fotográfica</span>
                                </div>
                            </template>
                        </div>

                        <div class="bg-gray-50 dark:bg-black/20 p-5 rounded-2xl border-l-4 border-smart-cobalto">
                        ...
                        <template x-if="reporteSeleccionado.estado === 'resuelto'">
                            <div class="mt-4 pt-6 border-t dark:border-white/5">
                                ...
                                <template x-if="reporteSeleccionado.evidencia_path">
                                    <div class="w-full h-48 bg-gray-100 dark:bg-black/40 rounded-2xl mb-4 border dark:border-white/5 overflow-hidden">
                ...
                {{-- Acciones del Admin --}}
                <div class="p-6 bg-gray-50 dark:bg-black/40 border-t dark:border-white/5 shrink-0">
                    <template x-if="reporteSeleccionado.estado === 'pendiente'">
                        <form :action="'/admin/incidencia/' + reporteSeleccionado.id + '/estado'" method="POST">
                            ...
                            <button type="submit" class="w-full bg-smart-warning text-white font-black py-4 rounded-2xl transition-all uppercase text-xs tracking-[0.1em] shadow-lg shadow-smart-warning/20 hover:brightness-110 active:scale-95 flex justify-center items-center gap-2">
                                <span class="material-symbols-outlined">engineering</span> Asignar a Cuadrilla
                            </button>
                        </form>
                    </template>
                    ...
                    <template x-if="reporteSeleccionado.estado === 'en proceso'">
                        <form :action="'/admin/incidencia/' + reporteSeleccionado.id + '/estado'" method="POST" enctype="multipart/form-data" class="space-y-4">
                            ...
                            <div class="space-y-4">
                                <textarea name="comentario_admin" rows="2" class="w-full bg-white dark:bg-black/40 border-gray-200 dark:border-white/10 text-gray-700 dark:text-white rounded-xl focus:ring-smart-success text-sm placeholder-gray-400 font-medium" placeholder="Escribe el mensaje de cierre para el ciudadano..."></textarea>
                                ...
                                <div class="flex items-center gap-4">
                                    <label class="flex-1">
                                        ...
                                        <input type="file" name="evidencia" accept="image/*" @change="const file = $event.target.files[0]; if(file) previewUrl = URL.createObjectURL(file);" 
                                               class="block w-full text-xs text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:uppercase file:bg-smart-cobalto/10 file:text-smart-cobalto hover:file:bg-smart-cobalto/20 cursor-pointer">
                                    </label>
                            ...
                            <button type="submit" class="w-full bg-smart-success text-white font-black py-4 rounded-2xl transition-all uppercase text-xs tracking-[0.1em] shadow-lg shadow-smart-success/20 hover:brightness-110 flex justify-center items-center gap-2">
                                <span class="material-symbols-outlined text-lg">task_alt</span> Resolver y Notificar
                            </button>
                        </form>
                    </template>
                </div>
    ...
    <script>
        ...
        function cargarMapa(lat, lng) {
            if (!adminMap) {
                adminMap = L.map('mapAdmin', { zoomControl: false }).setView([lat, lng], 18);
                L.control.zoom({ position: 'bottomright' }).addTo(adminMap);
                
                // Capa satelital híbrida para mejor contexto
